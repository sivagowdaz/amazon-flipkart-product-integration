...
def query_page(request):
    ...
    if request.POST.get('index_value_f') and len(flipkart_products) != 0:
        ...
        if required_data["product_description"]:
            new_product_description = '|'.join(required_data["product_description"])
        else:
        ...
        for data in newdata:
            ...
            if int(id) < 99:
                ...
                if product['product_heading']:
                    pdf.cell(10, 5, txt=product["product_heading"], ln=line_num, align='c')
                    line_num += 1
                if product['product_ratting']:
                    pdf.cell(10, 5, txt=product["product_ratting"], ln=line_num, align='c')
                    line_num += 1
                if product['user_review_text']:
                    pdf.cell(10, 5, txt=product["user_review_text"], ln=line_num, align='c')
                    line_num += 1
                ...
                if product['original_price']:
                    ...
                    pdf.cell(10, 5, txt=modified_or_price, ln=line_num, align='c')
                    line_num += 1
                ...
                if product['product_description']:
                    pdf.cell(10, 5, txt='product description:', ln=line_num, align='c')
                    line_num += 1
                ...
                if product['product_description']:
                    ...
                    for desc in desc_array:
                        ...
                        line_num += 1
                if product['ratting']:
                    pdf.cell(10, 5, txt=product["ratting"], ln=line_num, align='c')
                    line_num += 1
                if product['total_review']:
                    pdf.cell(10, 5, txt=product["total_review"], ln=line_num, align='c')
                    line_num += 1
                if product['product_price']:
                    pdf.cell(10, 5, txt=product["product_price"], ln=line_num, align='c')
                    line_num += 1  
                if os.path.exists(file_name):
                    os.remove(file_name)
                count += 1
    ...
    if request.POST:
        if request.POST.get('fproductid'):
            ...
            if email and password and product_identifier:
                required_data_login = flipkart_products[int(product_identifier)]
                identifier_text = required_data_login["product_heading"]
                login_to_flipkart(email, password, search_string, identifier_text)
        ...
        if request.POST.get('aproductid'):
            ...
            if email and password and product_identifier:
                required_data_login = amazon_products[int(product_identifier) - 100]
